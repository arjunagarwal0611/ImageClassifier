...
def perform_tqdm_default_out_stream_hack(tqdm_file_stream, tqdm_nb_columns=None):
    ...
    class TQDMPatch(tqdm.orignal_class):
        ...
        def __init__(self, iterable=None, desc=None, total=None, leave=True,
                     ...
                     bar_format=None, initial=0, position=None, postfix="Done",
...
class StdOutTextQueueReceiver(QObject):
    ...
    @pyqtSlot()
    def run(self):
        self.queue_std_out_element_received_signal.emit('--->Progress appears here <---\n')
        while True:
            text = self.queue.get()
...
class TQDMTextQueueReceiver(QObject):
    ...
    def run(self):
        ...
        self.queue_tqdm_element_received_signal.emit('\r---> Engine Started <---\n')
        while True:
            text = self.queue.get()
